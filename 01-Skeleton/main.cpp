...
void LoadResources()
{
	...
	for ( i = 0;i<=10;i++)
	{
		if (i>0 && i <= 3)
		{
			...
			object[i]->SetPosition(10.0f, 130.0f+30*i);
		}
		else if (i > 3 && i < 6)
		{
			object[i] = new CGameObject(BRICK_TEXTURE_PATH);
			object[i]->SetPosition(10.0f + 30 * i, 50.0f);
		}
		else
		{
			object[i] = new CBall(BALL_TEXTURE_PATH);
			object[i]->SetPosition(10.0f+30*i, 30.0f+30*i);
		}
	}
}
